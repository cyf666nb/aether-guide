# SCORE-IMPACT: MVP acceptance tests for session, trace, error, and WS demo loop.
from aether_api.main import app
from fastapi.testclient import TestClient


def test_create_session_has_trace() -> None:
    with TestClient(app) as client:
        response = client.post(
            "/api/v1/sessions",
            headers={"X-Trace-Id": "trace-test"},
            json={
                "scenic_id": "demo-scenic",
                "user_id": "visitor-1",
                "locale": "zh-CN",
                "idempotency_key": "test-session-1",
            },
        )
    assert response.status_code == 200
    payload = response.json()
    assert payload["code"] == "OK"
    assert payload["trace_id"] == "trace-test"
    assert payload["data"]["scenic_id"] == "demo-scenic"


def test_landmarks_seed_loads() -> None:
    with TestClient(app) as client:
        response = client.get("/api/v1/landmarks?scenic_id=demo-scenic")
    assert response.status_code == 200
    payload = response.json()
    assert len(payload["data"]["landmarks"]) >= 3
    assert payload["data"]["landmarks"][0]["name"] == "月门听泉"


def test_validation_error_is_uniform() -> None:
    with TestClient(app) as client:
        response = client.post("/api/v1/sessions", json={"scenic_id": ""})
    assert response.status_code == 422
    payload = response.json()
    assert payload["code"] == "VALIDATION_ERROR"
    assert "trace_id" in payload


def test_websocket_echo_loop() -> None:
    with TestClient(app) as client:
        created = client.post(
            "/api/v1/sessions",
            json={
                "scenic_id": "demo-scenic",
                "user_id": "visitor-2",
                "locale": "zh-CN",
                "idempotency_key": "test-session-2",
            },
        )
        session_id = created.json()["data"]["id"]
        with client.websocket_connect(f"/api/v1/sessions/{session_id}/stream") as websocket:
            websocket.send_json({"type": "user_text", "text": "介绍墨桥", "locale": "zh-CN"})
            message = websocket.receive_json()

    assert message["code"] == "OK"
    assert message["data"]["session_id"] == session_id
    assert "导览回声" in message["data"]["content"]
